...
@Slf4j
@Component
public @Data class ScieloContentIndexableMapCreatorFromNaiaraTSVs {
	...
	public Map<String, Object> processAndCreateMapToIndex(String dublinCoreContent, String rawContent,
		...
		Set<String>umlsConceptNames=Sets.newHashSet();
		for (String umlsContent : umlsProcessedContents) {
			log.info("Parsing TSVs with umls info...");
			try {
				String[] umlsInfoLines = umlsContent.split("\r?\n");
				for (String umlsInfoLine : umlsInfoLines) {
					String[] umlsInfo = umlsInfoLine.split("\t");
					String umlsCode = umlsInfo[0];
					String umlsConceptName = umlsInfo[1];
					umlsConcepts.add(umlsCode);
					umlsConceptNames.add(umlsConceptName);
				}
